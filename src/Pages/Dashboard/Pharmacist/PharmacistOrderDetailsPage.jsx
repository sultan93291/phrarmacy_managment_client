...
const PharmacistOrderDetailsPage = () => {
  ...
  <div className="text-[#052D4C] mt-7 space-y-3">
    {[
      { label: "Order ID", value: `#${MedicindeInfo?.order?.details?.order_id}` },
      { label: "Name", value: MedicindeInfo?.order?.details?.name },
      { label: "Email", value: MedicindeInfo?.order?.details?.email },
      { label: "Date of Birth", value: formatDate(MedicindeInfo?.order?.details?.date_of_birth) },
      { label: "Gender", value: loggedInUser?.gender ? loggedInUser.gender : "Not selected yet" },
      ...
      <div key={index} className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <p className="text-base lg:text-lg font-bold">{item.label}:</p>
  ...
  <div className="mt-12 w-full">
    ...
    {MedicindeInfo?.order?.order_items?.map(med => (
      <div key={med.medicine} className="grid grid-cols-1 sm:grid-cols-4 gap-4 py-4 border-b border-[#E7EBF4] text-base">
        {/* Description */}
        <div className="text-start space-y-1">
          <h2 className="font-bold">{med?.medicine}</h2>
          <p className="text-sm text-gray-500">{med?.quantity} Medicine included</p>
        </div>
        ...
        <h2 className="hidden sm:block text-center">{med?.quantity}</h2>
        <h2 className="hidden sm:block text-center">$ {med?.unit_price}</h2>
        <h2 className="hidden sm:block text-right">$ {med?.total_price}</h2>
      </div>
    ))}
  </div>
</div>
  ...
  <Accordion type="single" collapsible className="mt-10 space-y-3">
    {MedicindeInfo?.past_orders?.map((item) => (
      <AccordionItem key={item?.details?.order_id} value={item?.details?.order_id} className="border-b-0 bg-white px-5 rounded-lg">
        <AccordionTrigger>
          {/* Header */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-10 text-[#052D4C]">
            <h2 className="text-base sm:text-lg font-semibold">Order ID :</h2>
            <p className="font-medium text-sm sm:text-base">#{item?.details?.order_id}</p>
          </div>
        </AccordionTrigger>
        ...
        <AccordionContent>
          <div className="bg-white rounded-lg py-6 px-4">
            {/* Title */}
            <h2 className="text-xl sm:text-2xl font-bold text-[#052D4C]">Order Details</h2>

            {/* Description */}
            <div className="text-[#052D4C] mt-5 space-y-3">
              {[
                { label: "Order ID", value: `#${item?.details?.order_id}` },
                { label: "Name", value: item?.details?.name },
                { label: "Email", value: item?.details?.email },
                { label: "Date of Birth", value: formatDate(item?.details?.date_of_birth) },
                { label: "Gender", value: loggedInUser?.gender || "Gender not selected" },
                { label: "Phone", value: item?.details?.phone }
              ].map((info, index) => (
                <div key={index} className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm sm:text-lg font-bold">{info.label}:</p>
        ...
        {/* right content */}
        <div className="px-6 order-1 lg:order-2 sm:px-12 py-12 sm:py-16 w-full sm:w-2/5 bg-white rounded-lg h-fit">
  {/* Title */}
  <h3 className="text-lg sm:text-xl font-bold text-[#052D4C] text-center">Manage the order</h3>
