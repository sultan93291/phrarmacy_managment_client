...
function MedicineDetails({ data }) {
  ...
  const [subtotalPrice, setSubTotalPrice] = useState(data?.price || 0); // Initialize with default
  const [quantity, setQuantity] = useState(data?.quantity || 0); // Rename to quantity
  ...
  const handleValueChange = value => {
    ...
    const quantity = parseInt(data?.quantity * value) || 0;
    const subTotalPrice = data?.price * value || 0;
    ...
    if (counsultainid) {
      if (assesMentResult.assesmentResult) {
        ...
        localStorage.setItem(
          ...
          `/treatment/consultation/${counsultainid}`
        );
